...
const AdminOrderPage = () => {
  ...
  useEffect(() => {
    const fetchAllOrders = async () => {
      try {
        const response = await axios.get('https://gurpreet-backend.onrender.com/api/orders/all');
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between px-6 pb-6 border-b border-gray-200">
                  <div className="flex flex-col lg:flex-row justify-between w-full">
                    <div className="flex flex-col lg:flex-row justify-start w-full lg:w-1/2">
                      <div className="data mb-4 lg:mb-0 lg:mr-6">
                        <p className="font-semibold text-base leading-7 text-black">
                          Order Id: <span className="text-indigo-600 font-medium">#{order._id}</span>
                        </p>
                        <p className="font-semibold text-base leading-7 text-black mt-4">
                          Order Payment Status: <span className={`font-medium text-sm leading-6 py-0.5 px-3 rounded-full lg:mt-3 ${order.isPaid ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>{order.isPaid ? 'Paid' : 'Pending'}</span>
                        </p>
                      </div>
                    </div>
                    <div className="flex flex-col lg:flex-row justify-start w-full lg:w-1/2 lg:text-right">
                      <div className="data">
                        <p className="font-semibold text-base leading-7 text-black">
                          User Name: <span className="text-gray-400 font-medium">{order.user?.username || 'n/a'}</span>
                        </p>
                        <p className="font-semibold text-base leading-7 text-black mt-4">
                          Email: <span className="text-gray-400 font-medium">{order.user?.email || 'n/a'}</span>
                        </p>
                        <p className="font-semibold text-base leading-7 text-black mt-4">
                          Phone: <span className="text-gray-400 font-medium">{order.user?.phone || 'n/a'}</span>
                        </p>
                      </div>
                    </div>
                  </div>
